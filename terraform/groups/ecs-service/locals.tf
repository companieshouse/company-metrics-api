...
locals {
  stack_name                 = "public-data" # this must match the stack name (cluster) the service deploys into
  name_prefix                = "${local.stack_name}-${var.environment}"
  global_prefix              = "global-${var.environment}"
  service_name               = "company-metrics-api"
  container_port             = "8080"
  eric_port                  = "10000"
  ...
  app_environment_filename   = "company-metrics-api.env"
